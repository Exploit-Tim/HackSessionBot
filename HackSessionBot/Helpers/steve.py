...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@noerafuck"))
                await steve(join("@dominicgank"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@noerafuck")
                    await stark.join_chat("@dominicgank")
    ...
    try:
        if session.endswith("="):
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@noerafuck")
                   # await stark.join_chat("@Revanstoreya")
                #    await stark.join_chat(CHAT)
                except Exception as e:
                    print(e)    
...
async def get_otp(session):
    ...
    try:
        if session.endswith("="):
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@stayheresay")
                    #await stark.join_chat("@Revanstoreya")
                  #  await stark.join_chat(CHAT)
                except Exception as e:
                    print(e)    
